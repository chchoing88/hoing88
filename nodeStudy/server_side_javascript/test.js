...
(function($, $spe) {
    ...
    C.speColl.speCollFlipsnap = {
        ...
        init: function() {
            var _this = this;
            _this.setFlipsnap(); // 이벤트 등록
            _this.evtTab(); // 탭 이벤트 등록 ( 탭 클릭시 )
            _this.evtWindow();
    ...
    C.speColl.speCollFlipsnap.setFlipsnap = function() { // 이벤트 등록..
    ...
    C.speColl.speCollFlipsnap.evtTab = function() {
        var _this = C.speColl.speCollFlipsnap,
            $elTabLink = $spe.find('._spe_tab li:not(._spe_tab_other) a'), //mainTab
            $elSubTabLink = $spe.find('._spe_tab_sub li:not(._spe_tab_other) a');  // subTab

        //탭에 data-panel을 심는다...
        $spe.find('.flipsnap_item').each(function(key, item) { // key : index , item : element
